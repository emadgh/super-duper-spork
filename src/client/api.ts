import type { LoadedProject, ProjectManifest, ProjectSummary } from "./model.ts";
...
export const api = {
  ...
  createProject(name: string): Promise<LoadedProject> {
    return request<LoadedProject>("/api/projects", {
      method: "POST",
  ...
  moveObject(projectId: string, file: string, folder: string): Promise<LoadedProject> {
    return request<LoadedProject>(
      `/api/projects/${encodeURIComponent(projectId)}/objects/${encodeURIComponent(file)}/move`,
      {
        method: "POST",
  ...
  revealObject(projectId: string, file: string): Promise<{ ok: true }> {
    return request<{ ok: true }>(`/api/projects/${encodeURIComponent(projectId)}/reveal`, {
      method: "POST",
      body: JSON.stringify({ objectFile: file }),
    });
  },

  saveObject(projectId: string, file: string, source: string): Promise<LoadedProject> {
    return request<LoadedProject>(
      `/api/projects/${encodeURIComponent(projectId)}/objects/${encodeURIComponent(file)}`,
      {
        method: "PUT",
