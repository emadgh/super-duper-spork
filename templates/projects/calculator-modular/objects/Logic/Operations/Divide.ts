export default defineObject({
      name: "DivideCommand",
      description: "Divide is isolated in its own command file.",
      actions: {
        select: {
        ...
        apply: {
          ...
          run(_context, inputs) {
            ...
            const left = Number(inputs.left ?? 0); const right = Number(inputs.entry ?? 0); const result = right === 0 ? Number.NaN : left / right;
